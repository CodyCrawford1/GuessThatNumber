...
while (restartGame){
    // Asks user to enter a to set the upper bound for the random number that will be created.
    rangeNum = prompt(`Please enter a maximum number for the range`);

    // Using parseInt to attempt to convert the user's response into a number value
    rangeNum = parseInt(rangeNum);

    // Verifies the user's entry for a range number is a number greater than zero (NOTE: All number, positive or negative, have default boolean value of true, except for zero which has a default boolean value of false. Also, NaN has a default boolean value of false as well.)
    ...
    prompt(prompt(`Please enter a number of attempts allowed:`));

    // Verifying the users entry for a number of attempts allowed is a number greater than zero and less than the range they set
    ...
    // Continues looping until the user guesses the correct number or runs ou of attempts (NOTE: Loops until a BREAK keyword is run)
    while (true){
    ...
    // Verifies the user's guess is number greater than zero and less than or equal to the range they set
  ...
  while (true){
    // Checks if the user's answer is no (AKA `N`)
    if (playAgain === `N`) {
    ...
    //  Checks if the user's answer is yes (AKA `Y`)
    } else if (playAgain === `Y`) {
    // The game restarts
      break;
